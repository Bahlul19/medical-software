...
$sql = "SELECT id,role,clinic,facility,name_f,name_l,uname,phone_1,phone_2,email,addr_1,addr_2,addr_city,addr_state,addr_zip,date_hire,edu FROM users ";
...
echo "<table class='table InterperterRecordsData main-table' border = '1' id = 'export_table'>";
echo "<tr>";
...
while ($row = mysql_fetch_assoc($go)){
	...
	if(($rosEXP != 'NO' && $isInt == 5) || ($rosEXP == 'NO' && $isInt != 5)) {
		if($rosEXP == 'YES'){
