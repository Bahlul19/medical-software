...
formForm("{$htmlRoot}/proc/appointmentCancel.php",'post','onsubmit="return validateForm();"');
...
$rbf = $rBy['name_f'];
$rbl = $rBy['name_l'];
formLabel("cancelled_by","Cancellation Requested By");
...
formLabel('sendEmail','Cancel this Appointment');
formInput('iAgree','checkbox','ON',"id = 'iAgree'");
clearfix();


clearfix();
formInput('submit','submit','Submit Cancellation Request','id="submit" class = "nextButton"');
...
function validateForm(){
	...
	var $string = ' ';
	if($cb == ''){
		$string=$string+' You must sign your name in the "Cancellation Requested By" field. ';
		$count++;
